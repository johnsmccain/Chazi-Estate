...
export const useHedera = (): HederaHook => {
  ...
  const createLoan = useCallback(async (params: CreateLoanParams) => {
    ...
    try {
      const response = await apiService.createLoan(params);
  ...
  const makePayment = useCallback(async (params: MakePaymentParams) => {
    ...
    try {
      const response = await apiService.makePayment(params);
      ...
      if (response.success && response.data) {
        ...
        throw new Error(response.error || 'Failed to make payment');
      }
    } catch (error) {
      setState(prev => ({
        ...prev,
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to make payment' };
    }
  }, []);

  const calculateTotalOwed = useCallback(async (loanId: string) => {
    try {
      const response = await apiService.calculateTotalOwed(loanId);
      
      if (response.success && response.data) {
        return { success: true, data: response.data };
      } else {
        return { success: false, error: response.error || 'Failed to calculate total owed' };
      }
    } catch (error) {
      return { success: false, error: error instanceof Error ? error.message : 'Failed to calculate total owed' };
    }
  }, []);
  ...
  const addRevenue = useCallback(async (params: AddRevenueParams) => {
    ...
    try {
      const response = await apiService.addRevenue(params);
      ...
      if (response.success && response.data) {
        ...
        throw new Error(response.error || 'Failed to add revenue');
      }
    } catch (error) {
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : 'Failed to add revenue',
        isLoading: false,
      }));
      return { success: false, error: error instanceof Error ? error.message : 'Failed to add revenue' };
    }
  }, []);
  ...
  const distributeRevenue = useCallback(async (propertyId: string) => {
    ...
    try {
      const response = await apiService.distributeRevenue(propertyId);
      ...
      if (response.success && response.data) {
        ...
        throw new Error(response.error || 'Failed to distribute revenue');
      }
    } catch (error) {
      setState(prev => ({
        ...prev,
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to distribute revenue' };
    }
  }, []);

  const getTotalRevenue = useCallback(async (propertyId: string) => {
    try {
      const response = await apiService.getTotalRevenue(propertyId);
      
      if (response.success && response.data) {
        return { success: true, data: response.data };
      } else {
        return { success: false, error: response.error || 'Failed to get total revenue' };
      }
    } catch (error) {
      return { success: false, error: error instanceof Error ? error.message : 'Failed to get total revenue' };
    }
  }, []);
  ...
  const getTokenBalance = useCallback(async (accountId: string, tokenId: string) => {
    try {
      const response = await apiService.getTokenBalance(accountId, tokenId);
      
      if (response.success && response.data) {
        return { success: true, data: response.data };
      } else {
        return { success: false, error: response.error || 'Failed to get token balance' };
      }
    } catch (error) {
      return { success: false, error: error instanceof Error ? error.message : 'Failed to get token balance' };
    }
  }, []);
  ...
  const transferTokens = useCallback(async (from: string, to: string, tokenId: string, amount: string) => {
    ...
    try {
      const response = await apiService.transferTokens(from, to, tokenId, amount);
      ...
      if (response.success && response.data) {
        ...
        throw new Error(response.error || 'Failed to transfer tokens');
      }
    } catch (error) {
      setState(prev => ({
        ...prev,
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to transfer tokens' };
    }
  }, []);
  ...
  const getTransactionStatus = useCallback(async (txHash: string) => {
    try {
      // For now, return a mock status - in production, this would query the blockchain
      return {
        hash: txHash,
